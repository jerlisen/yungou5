...
class home extends base {
	...
	public function mobilecheck(){	
		...
		if(isset($_POST['submit'])){
			...
			//淘豆、经验添加			
			$isset_user=$this->db->GetList("select `uid` from `@#_member_account` where `content`='手机认证完善奖励' and `type`='1' and `uid`='$member[uid]' and (`pay`='经验' or `pay`='淘豆')");	
			if(empty($isset_user)){
				$config = System::load_app_config("user_fufen");//淘豆/经验
				$time=time();
				$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[uid]','1','淘豆','手机认证完善奖励','$config[f_phonecode]','$time')");
...
//淘豆、经验添加			
			...
			if(empty($isset_user)){
				...
				$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[uid]','1','淘豆','手机认证完善奖励','$config[f_overziliao]','$time')");
	...
	public function usermodify(){
		...
		if(isset($_POST['submit'])){			
			...
			//淘豆、经验添加
			$isset_user=$this->db->GetOne("select `uid` from `@#_member_account` where (`content`='手机认证完善奖励' or `content`='完善昵称奖励') and `type`='1' and `uid`='$member[uid]' and (`pay`='经验' or `pay`='淘豆')");	
			if(!$isset_user){			
				$config = System::load_app_config("user_fufen");//淘豆/经验
				$time=time();

				$this->db->Query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[uid]','1','淘豆','完善昵称奖励','$config[f_overziliao]','$time')");
	...
	public function userfufen(){
		$member=$this->userinfo;	
		$uid = $member['uid'];
		$title="账户淘豆 - "._cfg("web_name");
	
		$total=$this->db->GetCount("select * from `@#_member_account` where `uid`='$uid' and `pay` = '淘豆'");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,20,$pagenum,"0");		
		$account = $this->db->GetPage("select * from `@#_member_account` where `uid`='$uid' and `pay` = '淘豆' ORDER BY time DESC",array("num"=>20,"page"=>$pagenum,"type"=>1,"cache"=>0));
	...
	public function qiandao() {
		...
		if ( isset($_POST['submit']) ) {
			...
			if ( $member['sign_in_date'] == date('Y-m-d') ) {
				_message("您今天已经签到过了。",WEB_PATH."/member/home/qiandao");
				...
				if ( $member['sign_in_date'] == date('Y-m-d',strtotime('-1 day')) ){
					...
					$mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '淘豆', '每日签到', '$time_score', '".time()."')");
					...
					if ( $big ) {
						$mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '淘豆', '签到大礼包', '$time_day_score', '".time()."')");
						_message("签到成功，成功领取{$time_score}淘豆。<br />恭喜您获得{$time_day_score}淘豆的大礼包。<br />您的当前淘豆为{$score}",WEB_PATH."/member/home/qiandao",10);
					} else {
						_message("签到成功，成功领取{$time_score}淘豆。<br />您的当前淘豆为{$score}。<br />再连续签到".($time_day-$sign_in_time)."天就能领取大礼包啦，加油！！！",WEB_PATH."/member/home/qiandao");
					}
					...
					$mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '淘豆', '每日签到', '$time_score', '".time()."')");
					$mysql_model->Query("UPDATE `@#_member` SET score='".$score."',sign_in_time='".$sign_in_time."', sign_in_time_all='".$sign_in_time_all."', sign_in_date='".$sign_in_date."' where uid='".$member['uid']."'");
					_message("签到成功，成功领取{$time_score}淘豆。<br />您的当前淘豆为{$score}",WEB_PATH."/member/home/qiandao");
				}
			}
			die;
		}
