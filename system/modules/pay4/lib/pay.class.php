...
class pay {
	...
	**/
	...
	private function jf_pay_bag(){
		...
		if($this->fufen){
			$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '淘豆', '购买了商品', '{$this->fufen}', '$time')");
		}
	...
	**/
	...
	private function pay_bag(){
		...
		if(!$this->fufen_to_money){
			...
			$query_add_fufen_2 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '淘豆', '$mygoscore_text', '$mygoscore', '$time')");
