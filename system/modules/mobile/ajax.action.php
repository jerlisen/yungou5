...
# 我的修改
	...
	public function getLotteryList(){
	   ...
	   $shopsum=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is not null");

	   //最新揭晓
		$shoplist['listItems']=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is not null ORDER BY `q_end_time` DESC limit $FIdx,$EIdx");
