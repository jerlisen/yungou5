...
public function address(){
	...
	public function zhuanzhang(){
		...
		if($_POST['submit1']){
			...
			if(empty($_POST['money']) || $_POST['money']<10){



				_message("请输入转账金额，且不能小于10元",WEB_PATH."/mobile/home/zhuanzhang");
	...
	public function qiandao() {
		...
		if ( isset($_REQUEST['submit']) || $this->segment(5)=='submit' ) {
			if ( !$member['mobile']) {
				...
				x__message("您今天已经签到过了。",WEB_PATH."/mobile/home/qiandao/".$this->segment(4));
				...
				if ( $member['sign_in_date'] == date('Y-m-d',strtotime('-1 day')) ){
					...
					if ( $big ) {
						...
						x__message("签到成功，成功领取{$time_score}淘豆。<br />恭喜您获得{$time_day_score}淘豆的大礼包。<br />您的当前淘豆为{$score}",WEB_PATH."/mobile/home/qiandao/".$this->segment(4),10);
					} else {
						x__message("签到成功，成功领取{$time_score}淘豆。<br />您的当前淘豆为{$score}。<br />再连续签到".($time_day-$sign_in_time)."天就能领取大礼包啦，加油！！！",WEB_PATH."/mobile/home/qiandao/".$this->segment(4));
					}
					...
					$mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '淘豆', '每日签到', '$time_score', '".time()."')");
					$mysql_model->Query("UPDATE `@#_member` SET score='".$score."',sign_in_time='".$sign_in_time."', sign_in_time_all='".$sign_in_time_all."', sign_in_date='".$sign_in_date."' where uid='".$member['uid']."'");
					x__message("签到成功，成功领取{$time_score}淘豆。<br />您的当前淘豆为{$score}",WEB_PATH."/mobile/home/qiandao/".$this->segment(4));
				}
			}
			die;
		}
